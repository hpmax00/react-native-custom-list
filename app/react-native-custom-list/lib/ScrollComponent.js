...
import {Animated, DeviceEventEmitter, PanResponder, ScrollView, View, Platform} from "react-native";
...
export default class ScrollComponent extends Component {
  ...
  onMoveShouldSetPanResponderCapture = (evt, gestureState) => {
    ...
    if (
        this.scrollContentHeight - this.props.pullDownDistance <=
    ...
    return (
        this.props.enableHeaderRefresh &&
        (this.state.l_onTopReached_down || this.state.l_onTopReached_up)
    );
  };
  ...
  onPanResponderMove = (evt, gestureState) => {
    ...
    let _translateY =
        Math.ceil(Math.abs(gestureState.dy)) * this.props.pullFriction;
    //下拉刷新
    if (this.state.l_onTopReached_down && gestureState.dy > 0) {
      this.state.p_currPullDistance =
          _translateY >= this.props.pullDownDistance
              ? this.props.pullDownDistance
              : _translateY;
      this.state.p_translateY.setValue(
          -this.props.pullDownDistance + this.state.p_currPullDistance
      );
    ...
    //上拉隐藏刷新面板
      ...
      this.state.p_currPullDistance =
          _currPullDistance <= 0 ? 0 : _currPullDistance;
      this.state.p_translateY.setValue(
          -this.props.pullDownDistance + this.state.p_currPullDistance
      );
      this._headerRefresh.setCurrOffset(this.state.p_currPullDistance, null);
    ...
    if (this.state.l_onTopReached_down) {
      if (this.state.p_currPullDistance < this.props.pullDownDistance) {
        Animated.timing(this.state.p_translateY, {
          ...
          this._headerRefresh.setCurrOffset(
              this.state.p_currPullDistance,
              null
          );
        });
        ...
        if (this.state.gestureStatus !== STATUS_HEADER_REFRESHING) {
          this.props.onHeaderRefreshing instanceof Function &&
  ...
  onScroll = e => {
    ...
    if (dragDirection === 1) {
      ...
      if (onDrag) {
        ...
        y <= this.props.pullDownDistance &&
          this._headerRefresh.setCurrOffset(
            this.props.pullDownDistance - y,
            null
          );
      }
      //交互操作之后，视图正在滚动
      ...
      //函数滚动，scrollTo，scrollTo不会触发 onMomentumScrollBegin
        ...
        if (gestureStatus === G_STATUS_NONE) {
          if (y === this.props.pullDownDistance) {
            ...
            this._scrollPosHandle = setTimeout(
              () => this._scrollToPos(0, this.props.pullDownDistance),
              80
            );
          }
        }
  ...
  onScrollBeginDrag = e => {
    ...
    if (contentOffset.y <= this.props.pullDownDistance) {
      ...
      if (movePageY > startPageY) {
        //下拉
        if (
          gestureStatus !== STATUS_HEADER_REFRESHING
        ) {
          this.state.dragDirection = 1;
    ...
    this.props.onScrollBeginDrag instanceof Function &&
  ...
  onScrollEndDrag = e => {
    ...
    this.props.onScrollEndDrag instanceof Function &&
  ...
  onMomentumScrollBegin = e => {
    ...
    this.props.onMomentumScrollBegin instanceof Function &&
  ...
  onMomentumScrollEnd = e => {
    ...
    this.props.onMomentumScrollEnd instanceof Function &&
  ...
  renderHeaderRefresh = () => {
    if (this.props.enableHeaderRefresh) {
      return (
        <HeaderRefresh
          ref={ins => (this._headerRefresh = ins)}
          headerRefresh={this.props.headerRefresh}
        />
      );
    }
  };
    ...
    return (
      <View style={{ flex: 1 }} {...this._panResponder.panHandlers}>
        ...
        >
          ...
          >
            <View
              ref={ref => this._headerRefreshContainer = ref}
